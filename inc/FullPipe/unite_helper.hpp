...
public:
    ...
    UniteHelper(std::string graphName, std::string graphPath, int32_t videoWidth, int32_t videoHeight, bool needPP,
                 int32_t inputSizeNN, int32_t outputSize, WorkloadID id = -1,  RemoteMemoryFd _remoteMemoryFd = 0);
    UniteHelper() = default;

    void callInferenceOnBlobs(RemoteMemoryFd remoteMemoryFd = 0, const std::vector<InfoROI_t>& vecROI = std::vector<InfoROI_t>{});
    void SetUp();
...
protected:
    ...
    WorkloadContext_Helper _workloadContextHelper{static_cast<WorkloadID>(-1l)};
