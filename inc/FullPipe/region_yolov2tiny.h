...
inline int yolov2(const float *data, int * shape4D, int * strides4D, float thresh, float nms, 
        int num_classes,
        int image_width, int image_height,
        float * result);
            
#define MAX_CLASSES 125
...
inline static void get_region_boxes(const float *predictions, int *shape4D,
                                    int *strides4D, int num_classes, float thresh,
                                    box *boxes, const float *anchors);
inline static float box_iou(box a, box b);
...
inline void do_nms_sort(box *boxes, int total, int classes, float thresh)
{
    int i, j, k;
    sortable_bbox *s =
...
inline static float box_intersection(box a, box b)
{
...
inline static float box_union(box a, box b)
{
...
inline void correct_region_boxes(box *boxes, int n, int w, int h, int netw, int neth,
                                 int relative)
{
    int i;
    int new_w = 0;
...
inline static float _sigmoid(float x) { return 1. / (1. + exp(-x)); }

inline static void _softmax(float *x, int cnt)
{
...
// predictions is a tensor of shape (13,13,5,25), but stride is 125,
inline static void get_region_boxes(const float *predictions, int *shape4D,
                                    int *strides4D, int num_classes, float thresh,
                                    box *boxes, const float *anchors)
{
...
inline int yolov2(const float *data, int *shape4D, int *strides4D,
