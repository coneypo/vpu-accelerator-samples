...
static void ExecuteLongTermTracking(const std::string& model_path, cv::VideoCapture* capture);
...
int main(int argc, char** argv)
{
    ...
    ExecuteLongTermTracking(FLAGS_m, &capture);
...
static void ExecuteLongTermTracking(const std::string& model_path, cv::VideoCapture* capture)
{
    try
    {
        ...
        while (capture->read(frame))
        {
            ...
            cv::putText(frame, "D: detection to feed the tracker", cv::Point(5, 15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8, cv::Scalar(255, 0, 255), 1);
