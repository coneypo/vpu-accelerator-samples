...
public:
    ...
    /**
     * Tracking ID.
     * This is only valid when 'status' is TrackingStatus::TRACKED or TrackingStatus::LOST.
 ...
 * Difference against long term tracking is that user should update the object's information periodically.
 * It is recommended to update the tracked object's information for every 10-20 frames.
 * Since short term tracking assumes that user update correct information of the object frequently, amount of computation is less than long term tracking.
 * Therefore, if you are able to run your detector continuously, it would be better to use short term tracking type.
 * On the other hand, if your detector is executed only one time for any reason, long term trcking type should be taken.
 * @n
 * Zero term tracking can be thought as association between a detected object and tracked object.
 * Detected objects should always be added when Trace method is invoked.
 * For each frame, detected objects are mapped tracked objects with this tracking type, which enables ID tracking of detected objects.
 * @n
 ...
 * @n@n
 * Following sample code shows hot to use long term tracking type.
 * Objects are added to ObjectTracker only at the beginning of tracking.
 * @code
    ...
    auto ot = ot_builder.Build(vas::ot::TrackingType::LONG_TERM);
...
public:
     ...
     * Destructor.
     */
    VAS_EXPORT virtual ~ObjectTracker();
...
public:
     ...
     * @n
     * The supported color formats are BGR, NV12 and BGRx.
     *
...
public:
     ...
     * @n
     * Valid range: 1 <= max_num_objects. Or it can be -1 if there is no limitation of maximum number.
     * @n
     * Default value is -1 which means there is no limitation.
     */
    int32_t max_num_objects;

    /**
     * Input color format vas::ColorFormat. Supports BGR, BGRX, NV12
     * @n
