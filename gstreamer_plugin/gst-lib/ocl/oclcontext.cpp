...
void OclContext::releaseMemoryCL(gpointer info)
{
...
OclContext::acquireVAMemoryCL(VideoSurfaceID* surface, const cl_uint num_planes, const cl_mem_flags flags)
{
    ...
    if (!plane || !m_device->clEnqueueAcquireVA_Intel(plane, &mem_info->cl_memory[0])) {
        g_free(mem_info);
        return NULL;
    }
...
void OclContext::releaseVAMemoryCL(gpointer info)
{
    OclCLMemInfo** mem_info = (OclCLMemInfo**)info;
...
OclDevice::OclDevice()
    ...
    , m_platform(0)
    , m_device(0)
    , clCreateFromVA_APIMediaSurfaceINTEL(0)
{
}
...
OclDevice::init()
{
    ...
    if (!clGetDeviceIDsFromVA_APIMediaAdapterINTEL || !clCreateFromVA_APIMediaSurfaceINTEL || !clEnqueueAcquireVA_APIMediaSurfacesINTEL || !clEnqueueReleaseVA_APIMediaSurfacesINTEL) {
        g_print("OclDevice: failed to get extension function\n");
        return FALSE;
    }
#endif
